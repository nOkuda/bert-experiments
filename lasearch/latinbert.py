...
class LatinBERT():
    ...
    def get_berts_and_spans(self, raw_sents):
        ...
        for b in range(len(batched_data)):
            ...
            b_size = size[0]
            features = {
                'input_ids': batched_data[b].to(self.device),
                'attention_mask': batched_mask[b].to(self.device),
                'transforms': batched_transforms[b].to(self.device),
            }
            berts = self.model.forward(features)['token_embeddings']
            berts = berts.detach()
            berts = berts.cpu()
